...
export function StudioCharacter({ id, game, fallback, onChew, onUnlock }: {
  ...
  useFrame(() => {
    ...
    const g = game.current, now = performance.now(), t = (now - g.bites[id]) / 1000;
    ...
    const solve = (arm: Arm, goal: THREE.Vector3, side: number) => {
      ...
      const d = THREE.MathUtils.clamp(p.direction.length(), Math.abs(arm.a - arm.b) + .002, arm.a + arm.b - .002);
      ...
      p.elbow.copy(p.shoulder).addScaledVector(p.direction, along).addScaledVector(p.pole, Math.sqrt(Math.max(0, arm.a * arm.a - along * along)));
    ...
    const orient = (angle: number, roll: number, out: THREE.Quaternion) => {
      ...
      out.copy(p.yaw).multiply(p.q).multiply(p.twist).multiply(r.right.bind);
    };
    const rolling = active ? ease((t - .40) / .15) * (1 - ease((t - .58) / .22)) : 0;
    orient(-Math.PI / 2 - lift * 1.15, rolling * 1.4, p.handQ);
    orient(-Math.PI / 2 - 1.15, 0, p.eatQ);
    ...
    if (active) {
      ...
      if (t < .20) p.goal.lerpVectors(p.ready, p.scoop, ease(t / .20));
      else if (t < .58) {
        p.goal.copy(p.scoop); p.point.set(Math.sin((t - .20) / .38 * Math.PI) * .075, .025 * rolling, .025 * Math.sin(t * 14)).applyQuaternion(p.yaw); p.goal.add(p.point);
    ...
    for (let pass = 0; pass < 3; pass++) {
      ...
      scene.updateMatrixWorld(true);
      let liftOut = 0;
      for (const mesh of r.skin) {
        mesh.skeleton.update();
        for (let i = 0; i < mesh.geometry.attributes.position.count; i += 32) {
          p.point.fromBufferAttribute(mesh.geometry.attributes.position, i); mesh.applyBoneTransform(i, p.point); mesh.localToWorld(p.point);
    ...
    if (food.current) {
      food.current.visible = active && t > .43 && t < .91;
  ...
  return <group name={`studio-character-${id}`}><primitive object={scene}/><group ref={food} visible={false}>
