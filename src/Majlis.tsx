...
export const Majlis = memo(function Majlis() {
  const backrest = useMemo(() => textile([2, 1]), []);
  const minimalSeat = useMemo(() => minimalTextile([2, 1]), []);
  ...
  const sideBackrest = useMemo(() => textile([2 * 5.9 / 1.35, .84 / .91]), []);
  const sideSeat = useMemo(() => minimalTextile([2 * 5.9 / 7.3, .85]), []);
  ...
  return <group name="jordanian-majlis">
    ...
    <Block at={[-1.4, 2.75, -3.93]} size={[1.43, 2.05, .16]} color="#64472f" rotation={[0, 0, -.035]} />
    <Block at={[-1.4, 2.75, -3.83]} size={[1.23, 1.82, .035]} color="#ffffff" map={wallRug} rotation={[0, 0, -.035]} />
    <group name="majlis-lamp-lantern" position={[3.10, 0.50, -3.10]} scale={[0.75, 0.75, 0.75]}>
      <Block at={[0, .05, 0]} size={[.43, .085, .38]} color="#312b23" />
      <Block at={[0, .67, 0]} size={[.44, .07, .4]} color="#312b23" />
      {[-1, 1].flatMap(x => [-1, 1].map(z => <Block key={`${x}${z}`} at={[x * .17, .36, z * .145]} size={[.035, .61, .035]} color="#302c26" />))}
      <mesh position={[0, .35, 0]}><boxGeometry args={[.29, .52, .24]} /><meshStandardMaterial color="#e5ad55" transparent opacity={.3} roughness={.3} /></mesh>
      <mesh position={[0, .34, 0]} scale={[.05, .20, .05]}><sphereGeometry args={[1, 12, 8]} /><meshStandardMaterial color="#ffd889" emissive="#ffb543" emissiveIntensity={3} /></mesh>
      <mesh position={[0, .82, 0]} rotation={[0, Math.PI / 4, 0]}><coneGeometry args={[.34, .27, 4]} /><meshStandardMaterial color="#312b23" /></mesh>
      <mesh position={[0, 1.01, 0]}><torusGeometry args={[.055, .012, 8, 16]} /><meshStandardMaterial color="#312b23" /></mesh>
      <sprite position={[0,.36,.10]} scale={[.80,1.05,1]}><spriteMaterial map={glow} transparent depthWrite={false}/></sprite><pointLight position={[0, .47, .3]} intensity={7} distance={5} color="#ffb552" decay={2} />
    </group>
